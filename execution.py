...
def get_spot_existing_coins() -> set:
    """جلب العملات الموجودة مسبقاً في محفظة Spot (لحمايتها)"""
    try:
        params = {"timestamp": _ts()}
        params["signature"] = _sign(params, BINANCE_API_SECRET)
        r = requests.get(
...
def load_protected_coins():
    """استدعِها مرة واحدة عند بدء البوت لحفظ العملات الموجودة"""
...
def execute_signal(signal: dict) -> dict:
    ...
    if market_type == "FUTURES":
        ...
        usdt_to_use = round(balance * 0.10, 2)  # 10% من الرصيد المتاح
