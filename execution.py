...
def place_futures_order_and_sltp(symbol: str, direction: str, qty: float,
    ...
    try:
        ...
        print(f"Opening {direction} {symbol} qty={qty} SL={sl_price} TP={tp_price}")
        ...
        print(f"TP order: {tp_order.get('orderId')} ✅")
        ...
        print(f"Futures order error {symbol}: {e}")
    return {}
...
def place_spot_order(symbol: str, qty: float) -> dict:
    try:
        ...
        print(f"Spot BUY order: {order.get('orderId')} ✅")
        return order
    except BinanceAPIException as e:
        print(f"Spot API error {symbol}: {e}")
    except Exception as e:
        print(f"Spot order error {symbol}: {e}")
    return {}
