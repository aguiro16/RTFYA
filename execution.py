        ...
        for s in r.json().get("symbols", []):
            if s["symbol"] == symbol:
                ...
                for f in s.get("filters", []):
                    ...
                    if f["filterType"] in ("PRICE_FILTER", "PRICE_FILTER"):
...
def format_price(price: float, tick_size: str) -> str:
    """تنسيق السعر بناءً على tickSize الخاص بكل عملة"""
    decimals = len(tick_size.rstrip("0").split(".")[-1]) if "." in tick_size else 0
    rounded = round(price - (price % float(tick_size)), decimals)
...
def place_futures_sl_tp(symbol: str, direction: str, qty: float, sl: float, tp: float) -> bool:
    try:
        info       = get_symbol_info(symbol, "FUTURES")
        tick_size  = info["tickSize"]
        close_side = "SELL" if direction == "LONG" else "BUY"
        sl_price   = format_price(sl, tick_size)
        tp_price   = format_price(tp, tick_size)

        print(f"Placing SL at {sl_price}, TP at {tp_price} for {symbol}")
        ...
        print(f"SL order response: {sl_resp.json()}")
        ...
        print(f"TP order response: {tp_resp.json()}")
...
def execute_signal(signal: dict) -> dict:
    ...
    if market_type == "FUTURES":
        ...
        if order.get("orderId"):
            time.sleep(1)  # انتظر ثانية قبل وضع SL/TP
